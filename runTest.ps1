...
Write-Host 'La prueba se ejecutará con las siguientes especificaciones'
Write-Host 'ambiente: ' $ambiente
try {
    Write-Host '--> Si existe, se eliminarán los archivos dentro de las carpetas cypress/videos'
    rm -r cypress/videos -ErrorAction stop
    Write-Host '***Finalizado***' -ForegroundColor DarkGreen
}catch {
    Write-Warning "No existe el path cypress/videos"
}
try {
    Write-Host '--> Si existe, se eliminarán los archivos dentro de las carpetas cypress/screenshots'
...
try {
    Write-Host '--> Si existe, se eliminarán los archivos dentro de las carpetas reports/cucumber-htmlreport.html/features'
...
try {
	Write-Host '--> Si existe, se eliminará el archivo reports/cucumber-htmlreport.html/index.html'
...
try {
    ...
    cp cypress/fixtures/dataenviroment/$ambiente.json cypress/fixtures/dataenviroment/dataBase.json -ErrorAction stop
    Write-Host '***Finalizado***' -ForegroundColor DarkGreen
}catch {
